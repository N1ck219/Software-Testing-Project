...
# ALGORITHMS = ["DYNAMOSA", "MOSA", "MIO", "RANDOM"]
ALGORITHMS = ["DYNAMOSA", "MOSA", "MIO"]
NUM_RUNS = 2 if os.environ.get("CI") else 10
...
class ResourceMonitor:
    """Monitora l'uso di CPU e RAM di un processo e dei suoi figli."""
    def __init__(self, pid, interval=0.5):
        self.pid = pid
        self.interval = interval
        self.cpu_samples = []
        self.ram_samples = []  # In MB
    ...
    def _monitor(self):
        try:
            ...
            # Inizializza il monitoraggio CPU (la prima chiamata ritorna 0)
            main_process.cpu_percent(interval=None)
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    procs = [main_process] + main_process.children(recursive=True)
                    cpu_sum = 0
                    ram_sum = 0
                    ...
                    for p in procs:
                        try:
                            ...
                            ram_sum += p.memory_info().rss
                        except (psutil.NoSuchProcess, psutil.AccessDenied):
                            continue
                            
                    self.cpu_samples.append(cpu_sum)
                    self.ram_samples.append(ram_sum / (1024 * 1024))
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    break
    ...
    def get_stats(self):
        if not self.cpu_samples:
            return {"avg_cpu": 0.0, "peak_cpu": 0.0, "peak_ram": 0.0}
        return {
            "avg_cpu": round(sum(self.cpu_samples) / len(self.cpu_samples), 2),
            "peak_cpu": round(max(self.cpu_samples), 2),
            "peak_ram": round(max(self.ram_samples), 2)
        }

def run_pynguin(algorithm="DYNAMOSA"):
    """Esegue Pynguin per generare una nuova suite di test."""
    print(f"   🤖 Generazione test con Pynguin (Algoritmo: {algorithm})...")
    env = os.environ.copy()
    ...
    cmd = [
        ...
        "--algorithm", algorithm,
        "--maximum-search-time", "15",
        "--maximum-test-execution-timeout", "5",
        # "--test-case-minimization-strategy", "SIMPLE"
    ]
...
"""
    ...
    # Usiamo Popen per monitorare le risorse durante l'esecuzione
    process = subprocess.Popen(["mutmut", "run"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    print(f"      📊 Risorse: CPU Media {res_stats['avg_cpu']}%, RAM Picco {res_stats['peak_ram']}MB")
    ...
    return {
        ...
        "avg_cpu": res_stats['avg_cpu'],
        "peak_cpu": res_stats['peak_cpu'],
        "peak_ram": res_stats['peak_ram']
    }
...
def run_cosmic_ray(run_id):
    ...
    start_time = time.time()
    process = subprocess.Popen(["cosmic-ray", "exec", "cosmic-ray.toml", "session.sqlite"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def generate_comparison_report(data):
    ...
    avg_mut_cpu = sum(r['mutmut'].get('avg_cpu', 0) for r in data) / len(data)
    avg_cr_cpu = sum(r['cosmic_ray'].get('avg_cpu', 0) for r in data) / len(data)
    avg_mut_ram = sum(r['mutmut'].get('peak_ram', 0) for r in data) / len(data)
    avg_cr_ram = sum(r['cosmic_ray'].get('peak_ram', 0) for r in data) / len(data)

    # Preparazione dati per i grafici
    labels = [str(r['run_id']) for r in data]
    mut_scores = [r['mutmut']['mutation_score'] for r in data]
    cr_scores = [r['cosmic_ray']['mutation_score'] for r in data]
    
    mut_cpu = [r['mutmut'].get('avg_cpu', 0) for r in data]
    cr_cpu = [r['cosmic_ray'].get('avg_cpu', 0) for r in data]
    mut_ram = [r['mutmut'].get('peak_ram', 0) for r in data]
    cr_ram = [r['cosmic_ray'].get('peak_ram', 0) for r in data]
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <p>Confronto Strategie di Generazione (Pynguin)</p>
            </div>
            ...
            <div style="display: flex; justify-content: space-between; gap: 20px;">
                <div class="chart-container" style="flex: 1;">
                    ...
                    <p style="text-align: center; margin-top: 15px;">Media Mutmut: <strong>{avg_mut_cpu:.1f}%</strong> | Media Cosmic Ray: <strong>{avg_cr_cpu:.1f}%</strong></p>
                </div>
                <div class="chart-container" style="flex: 1;">
                    <h3>💾 Picco RAM (MB)</h3>
                    <canvas id="ramChart"></canvas>
                    <p style="text-align: center; margin-top: 15px;">Media Mutmut: <strong>{avg_mut_ram:.1f} MB</strong> | Media Cosmic Ray: <strong>{avg_cr_ram:.1f} MB</strong></p>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            new Chart(document.getElementById('cpuChart').getContext('2d'), {{
                ...
                data: {{
                    labels: {labels},
                    datasets: [
                        {{ label: 'Mutmut CPU %', data: {mut_cpu}, borderColor: '#3498db', backgroundColor: '#3498db22', fill: true, tension: 0.3 }},
                        {{ label: 'Cosmic Ray CPU %', data: {cr_cpu}, borderColor: '#e74c3c', backgroundColor: '#e74c3c22', fill: true, tension: 0.3 }}
                    ]
                }},
                options: {{ responsive: true, scales: {{ y: {{ beginAtZero: true, title: {{ display: true, text: 'CPU (%)' }} }} }} }}
            }});
            ...
            new Chart(document.getElementById('ramChart').getContext('2d'), {{
                ...
                data: {{
                    labels: {labels},
                    datasets: [
                        {{ label: 'Mutmut RAM (MB)', data: {mut_ram}, backgroundColor: '#3498db' }},
                        {{ label: 'Cosmic Ray RAM (MB)', data: {cr_ram}, backgroundColor: '#e74c3c' }}
                    ]
                }},
                options: {{ responsive: true, scales: {{ y: {{ beginAtZero: true, title: {{ display: true, text: 'RAM (MB)' }} }} }} }}
            }});
        </script>
    </body>
    </html>
    ...
    try:
        ...
        for algo in ALGORITHMS:
            for i in range(NUM_RUNS):
                run_id = f"{algo}_{i+1}"
                print(f"\n--- 🔄 Esecuzione Run {i+1}/{NUM_RUNS} - Algoritmo: {algo} ---")
                
                clean_run_cache()
                pynguin_time = run_pynguin(algo)
